  ...
  class WebSocketClient : IDisposable
  {
      ...
      switch (m_state)
      {
        case WebSocketClientState.Closed:
          ...
          if (ValidateClientHandshake(lines, out key))
          {
            ...
            m_streamSocket.Send(Identity.Data, Identity.Data.Length, false, true);
    ...
    private void OnMessage(object sender, MessageEventArgs e)
    {
      if (e.Opcode == OpcodeEnum.Close)
      {
        ...
        m_streamSocket.Send(Identity.Data, Identity.Size, false, true);
        m_streamSocket.Send("");
        m_state = WebSocketClientState.Closed;
      }
      else
      {
        m_incomingMessageQueue.Enqueue(new Message(Identity.Data, e.Payload));
      }
    }
    ...
    public bool Send(byte[] message, bool dontWait)
    {
      int frameSize = 2 + message.Length;
      ...
      if (message.Length <= 125)
      {
        frame[1] |= (byte)(message.Length & 127);
      }
      else
      {
        // TODO: implement
